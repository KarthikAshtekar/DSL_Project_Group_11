...
COLORS = ["#a78bfa", "#60a5fa", "#34d399", "#f59e0b", "#f472b6"]
...
<div class="dash-header">
  ...
  <p>Gurobi-powered workforce scheduling · Maximize job throughput across Fibre, Copper & Backlog</p>
</div>
""", unsafe_allow_html=True)
...
# OPTIMIZATION
# ─────────────────────────────────────────────
with st.spinner("Running Gurobi optimizer..."):
    model = gp.Model("Technician_Allocation")
    model.setParam("OutputFlag", 0)   # suppress Gurobi console output

    xF = model.addVars(I, J, vtype=GRB.INTEGER, name="Fibre")
    xB = model.addVars(I, J, vtype=GRB.INTEGER, name="Backlog")
    xC = model.addVars(I, J, vtype=GRB.INTEGER, name="Copper")

    # ── Objective ───────────────────────────
    model.setObjective(
        gp.quicksum(xF[i,j] + xB[i,j] for i in I for j in J)
    ...
    # ── Demand constraints ───────────────────
    for i in I:
        model.addConstr(gp.quicksum(xF[i,j] for j in J) <= F[i], f"FibreDemand_{i}")
        model.addConstr(gp.quicksum(xB[i,j] for j in J) <= B[i], f"BacklogDemand_{i}")
        model.addConstr(gp.quicksum(xC[i,j] for j in J) <= C[i], f"CopperDemand_{i}")

    # ── Capacity constraints ─────────────────
    for j in J:
        model.addConstr(
            gp.quicksum(t[i,j] * (xF[i,j] + xB[i,j] + xC[i,j]) for i in I)
            <= workers[j] * H,
            f"Capacity_{j}"
        )
        # Copper 2-day window
        model.addConstr(
            gp.quicksum(t[i,j] * (xB[i,j] + xC[i,j]) for i in I)
            <= 2 * workers[j] * H,
            f"CopperWindow_{j}"
        )
...
if model.status != GRB.OPTIMAL:
    st.error("❌ No optimal solution found. Try relaxing constraints or increasing technician counts.")
    st.stop()

# ── Extract values ───────────────────────────
total_fibre   = sum(int(xF[i,j].X) for i in I for j in J)
total_backlog = sum(int(xB[i,j].X) for i in I for j in J)
total_copper  = sum(int(xC[i,j].X) for i in I for j in J)
total_jobs    = total_fibre + total_backlog + total_copper

utilization   = []
used_hours    = []
for j in J:
    used = sum(t[i,j] * (xF[i,j].X + xB[i,j].X + xC[i,j].X) for i in I)
    cap  = workers[j] * H
...
with col3:
    ...
    ax3.bar(x - 0.2, demand,    0.35, label="Demand",    color="#334155", zorder=3)
    ax3.bar(x + 0.2, completed, 0.35, label="Completed", color=["#60a5fa","#f59e0b","#34d399"], zorder=3)
    ax3.set_xticks(x)
...
with tab1:
    rows = []
    for j in J:
        f_jobs = sum(int(xF[i,j].X) for i in I)
        b_jobs = sum(int(xB[i,j].X) for i in I)
        c_jobs = sum(int(xC[i,j].X) for i in I)
        total  = f_jobs + b_jobs + c_jobs
        util   = utilization[j-1]
        badge  = (
            '<span class="badge badge-error">Overloaded</span>' if util > 0.9 else
            '<span class="badge badge-warn">Efficient</span>'    if util > 0.7 else
            '<span class="badge badge-ok">Underutilized</span>'
        )
        rows.append(f"""
        <tr>
            <td><b>Type {j}</b></td>
...
with tab2:
    ...
    for i in I:
        done   = sum((xF[i,j].X + xB[i,j].X + xC[i,j].X) for j in J)
        demand = F[i] + B[i] + C[i]
        if done < demand:
    ...
    type_map = {"ok":"ok", "warn":"warn", "error":"error", "info":"info"}
    for kind, msg in insights:
        st.markdown(
            f'<div class="insight-card {type_map[kind]}">{msg}</div>',
            unsafe_allow_html=True
        )
...
<div style="text-align:center; color:#334155; font-family:'Space Mono',monospace; font-size:0.7rem; padding:1rem;">
    GUROBI OPTIMIZER · INTEGER PROGRAMMING · TECHNICIAN SCHEDULING
</div>
""", unsafe_allow_html=True)